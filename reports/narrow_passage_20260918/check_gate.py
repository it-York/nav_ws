...
ROOT=Path('/home/koala2/nav_ws');TMP=Path('/tmp/nav_ws_narrow_gate_check');TMP.mkdir(exist_ok=True)
...
obstacles=n.create_publisher(PointCloud2,'/cloud_obstacles',10)
...
def tick():
 ...
 obstacles.publish(create_cloud_xyz32(Header(stamp=t.header.stamp,frame_id='base_link'),[]))
...
results={}
for label,file in [('after',ROOT/'src/go2w_nav2_navigation/config/nav2_go2w.yaml')]:
 c=yaml.safe_load(file.read_text());c['map_server']['ros__parameters']['yaml_filename']=str(TMP/'map.yaml')
 l=c['local_costmap']['local_costmap']['ros__parameters'];l['plugins']=['static_layer','obstacle_layer','inflation_layer'];l['static_layer']={'plugin':'nav2_costmap_2d::StaticLayer','map_subscribe_transient_local':True}
